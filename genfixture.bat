...
IF EXIST "C:\Program Files\KiCad\6.0\bin" SET KICAD=C:\Program Files\KiCad\6.0\bin
...
"%KICAD%\python.exe" GenFixture.py --board %BOARD% --layer %LAYER% --rev %REV% --mat_th %MAT% --pcb_th %PCB% --out %OUTPUT% --screw_len %SCREW_LEN% --screw_d %SCREW_D% --washer_th %WASHER_TH% --nut_th %NUT_TH% --nut_f2f %NUT_F2F% --nut_c2c %NUT_C2C% --border %BORDER% --pogo-uncompressed-length %POGO_UNCOMPRESSED_LENGTH%